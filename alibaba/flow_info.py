...
class flow:
    ...
    def get_classi_count(self):
        ...
        sql = "select class_3,class_1,class_2 from crm_config where class_1 = '%s' and class_3 not in (%s)" % (self.all_classi,sql_detail)
        ...
        if result is None or len(result) == 0:
            ...
            if result is not None and len(result) > 0:
                my_sql.update("update crm_config_detail set updated_at = '%s' where id = %d" % (get_time(),result[0][0]))
        else:
            sql = "insert into crm_config_detail(classi,source,region,updated_at,count,status) values('%s','1688','%s','%s',1,0)" % (result[0][0],self.region,get_time())
    ...
    def search_classi_page(self,html,classi,page_size,index,class_id):
        ...
        for i in range(index,page_size):
            try:
                Mysql_db().update("update crm_config_detail set count = %d,updated_at ='%s' where id = %d" % (i,get_time(),class_id))
    ...
    def get_1688_reslut(self,company_name,html,dict,classi,result):
        ...
        results = parse_html.resultItem
        convert_result = json.dumps(results, ensure_ascii=False, encoding='UTF-8').encode("utf-8").replace("'",'"').replace(
    ...
    def get_yellow_page(self,company_name,html,dict,classi):
        ...
        convert_result = json.dumps(results, ensure_ascii=False, encoding='UTF-8').encode("utf-8").replace("'", '"').replace('\t', '').replace('\n', '').replace('\r', '').replace(' ', '').replace('"', '\\"')
    ...
    def insert_update_db(self,result,results,company_name,convert_result,classi):
        if result is not None and len(str(result)) > 2:
            if self.is_telephone(results.get('telphone')) > 0 or self.is_telephone(results.get('phone')) > 0:
                sql = 'update %s set clue_status = 3,1688_result = \"%s\", updated_at= \"%s\" where id=%d;' % ('1688_clue_new', convert_result, get_time(), int(result[0]))
            elif  self.is_telephone(results.get('contact_name'))> 0:
                sql = 'update %s set clue_status = 1,1688_result = \"%s\", updated_at= \"%s\" where id=%d;' % ('1688_clue_new', convert_result, get_time(), int(result[0]))
            Mysql_db().update(sql)
        else:
            if self.is_telephone(results.get('telphone')) > 0 or self.is_telephone(results.get('phone')) > 0:
                Mysql_db().insert("insert into test.1688_clue_new(company_name,clue_status,updated_at,1688_result,main_classi,page_home_url,page_achivers_url,classi_1,classi_2) values('%s',%d,'%s','%s','%s','%s',\'\','%s','%s')" % (company_name, 3, get_time(), convert_result, classi, results.get('url'), self.classi_1,self.classi_2))
            elif self.is_telephone(results.get('contact_name'))> 0:
                Mysql_db().insert("insert into test.1688_clue_new(company_name,clue_status,updated_at,1688_result,main_classi,page_home_url,page_achivers_url,classi_1,classi_2) values('%s',%d,'%s','%s','%s','%s',\'\','%s','%s')" % (company_name,1,get_time(),convert_result, classi, results.get('url'),self.classi_1,self.classi_2))
...
def processing():
    ...
    if user_password is None or len(user_password) == 0:
        user_password = (12334, 'flesdewe234', 'lqz123456789')
    try:
        print user_password
        if user_password is None or len(user_password) == 0:
